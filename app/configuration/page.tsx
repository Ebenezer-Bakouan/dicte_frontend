...
export default function ConfigurationPage() {
  ...
  const [parametresDictee, setParametresDictee] = useState<ParametresDictee>({
    niveau: 'moyen',
    sujet: '',
    longueurTexte: 'moyen',
  ...
  const handleNext = () => {
    if (isFormValid()) {
      // Convertir les données en chaînes pour les query parameters
      const queryParams = {
        ...Object.fromEntries(
          Object.entries(profilUtilisateur).map(([key, value]) => [key, String(value)])
        ),
        ...Object.fromEntries(
          Object.entries(parametresDictee).map(([key, value]) => [key, String(value)])
        )
      };

      const query = new URLSearchParams(queryParams).toString();
      router.push(`/dictation?${query}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <p>N&apos;oubliez pas de sauvegarder vos paramètres</p>
        <p>Vous n&apos;avez pas encore configuré vos paramètres</p>
        <p>L&apos;enregistrement a été sauvegardé avec succès</p>
      </div>
    </div>
  );
}